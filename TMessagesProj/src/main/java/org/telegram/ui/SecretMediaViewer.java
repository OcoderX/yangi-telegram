...
public class SecretMediaViewer implements NotificationCenter.NotificationCenterDelegate, GestureDetector.OnGestureListener, GestureDetector.OnDoubleTapListener {
    ...
    /** AyuGram: action bar menu id of the "Expire" button */
    private static final int ayu_expire_media = 100;
...
//        videoTextureView.setAlpha(videoCrossfadeAlpha = 0.0f);
    ...
    public void setParentActivity(Activity activity) {
        ...
        actionBar.setTitleRightMargin(dp(org.telegram.messenger.ayu.AyuConfig.showExpireButton ? 118 : 70));
        containerView.addView(actionBar, LayoutHelper.createFrame(LayoutHelper.MATCH_PARENT, LayoutHelper.WRAP_CONTENT));
        if (org.telegram.messenger.ayu.AyuConfig.showExpireButton) {
            org.telegram.ui.ActionBar.ActionBarMenu menu = actionBar.createMenu();
            org.telegram.ui.ActionBar.ActionBarMenuItem expireItem = menu.addItem(ayu_expire_media, R.drawable.msg_autodelete);
            expireItem.setContentDescription(LocaleController.getString(R.string.AyuExpireMedia));
        }
        ...
        containerView.addView(secretDeleteTimer, LayoutHelper.createFrame(119, 48, Gravity.TOP | Gravity.RIGHT, 0, 0, org.telegram.messenger.ayu.AyuConfig.showExpireButton ? 48 : 0, 0));
        ...
        if (!org.telegram.messenger.ayu.AyuConfig.allowScreenshotsInSecretChats) {
...
//        bottomSheet.scrollNavBar = true;
    ...
    public void openMedia(MessageObject messageObject, PhotoViewer.PhotoViewerProvider provider, Runnable onOpen, Runnable onClose) {
        ...
        org.telegram.messenger.ayu.AyuMessagesController.getInstance().onSelfDestructMediaOpened(messageObject.currentAccount, messageObject); //ayu
    ...
    /**
     * AyuGram: immediately expire the currently shown self-destructing media.
     * Marks the content as read (which starts the destroy countdown / makes the server expire it)
     * and closes the viewer. The read packet is forced through even in ghost mode.
     */
    private void expireCurrentMedia() {
        final MessageObject messageObject = currentMessageObject;
        if (messageObject != null && messageObject.messageOwner != null) {
            try {
                // "view once" media (ttl == 0x7FFFFFFF) is destroyed by the close action of the caller
                if (messageObject.messageOwner.ttl != 0x7FFFFFFF) {
                    ...
                    messageObject.messageOwner.destroyTime = connectionsManager.getCurrentTime();
                    messageObject.messageOwner.destroyTimeMillis = connectionsManager.getCurrentTimeMillis();
                    // force the read receipt through even when ghost mode blocks read packets
                    org.telegram.messenger.ayu.AyuState.setAllowReadPacket(dialogId, true, true);
                    if (org.telegram.messenger.DialogObject.isEncryptedDialog(dialogId)) {
                        MessagesController.getInstance(currentAccount).markMessageAsRead(dialogId, messageObject.messageOwner.random_id, 1);
                    } else if (messageObject.getId() > 0) {
                        MessagesController.getInstance(currentAccount).markMessageAsRead2(dialogId, messageObject.getId(), null, 0, 0, true);
                    }
                }
            } catch (Exception e) {
                FileLog.e(e);
