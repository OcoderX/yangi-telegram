...
public class StoryViewer implements NotificationCenter.NotificationCenterDelegate, BaseFragment.AttachedSheet, IPipSourceDelegate {
    ...
    boolean isShowing;
    /** set while re-entering open() after the ghost mode alert was answered */
    private boolean ayuSkipStoryAlert;
...
//            }
    ...
    public void open(int account, Context context, TL_stories.StoryItem storyItem, ArrayList<Long> peerIds, int position, StoriesController.StoriesList storiesList, TL_stories.PeerStories userStories, PlaceProvider placeProvider, boolean reversed) {
        ...
        if (org.telegram.messenger.ayu.AyuConfig.alertBeforeOpeningStory && org.telegram.messenger.ayu.AyuConfig.sendReadStories && !ayuSkipStoryAlert) {
            final int fAccount = account;
            final Context fContext = context;
            final TL_stories.StoryItem fStoryItem = storyItem;
            final ArrayList<Long> fPeerIds = peerIds;
            final int fPosition = position;
            final StoriesController.StoriesList fStoriesList = storiesList;
            final TL_stories.PeerStories fUserStories = userStories;
            final PlaceProvider fPlaceProvider = placeProvider;
            final boolean fReversed = reversed;
            org.telegram.ui.ayu.AyuGhostAlert.showStoryAlert(context, resourcesProvider, () -> {
                ayuSkipStoryAlert = true;
                open(fAccount, fContext, fStoryItem, fPeerIds, fPosition, fStoriesList, fUserStories, fPlaceProvider, fReversed);
                ayuSkipStoryAlert = false;
            });
            return;
        }
        ayuSkipStoryAlert = false;
