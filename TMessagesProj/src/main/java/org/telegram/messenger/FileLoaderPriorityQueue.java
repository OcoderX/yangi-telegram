...
public class FileLoaderPriorityQueue {
    ...
    private void checkLoadingOperationInternal() {
        ...
        if (SharedConfig.enableDownloadAccelerator) {
            max = type == TYPE_LARGE ? Math.max(MessagesController.getInstance(currentAccount).largeQueueMaxActiveOperations, SharedConfig.maxParallelActiveFiles) : Math.max(MessagesController.getInstance(currentAccount).smallQueueMaxActiveOperations, 8);
        } else {
            max = type == TYPE_LARGE ? MessagesController.getInstance(currentAccount).largeQueueMaxActiveOperations : MessagesController.getInstance(currentAccount).smallQueueMaxActiveOperations;
        }
