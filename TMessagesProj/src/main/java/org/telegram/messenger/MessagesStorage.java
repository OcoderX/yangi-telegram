...
*/
    ...
    public void deleteDialog(long did, int messagesOnly) {
        storageQueue.postRunnable(() -> {
            ayuSaveClearedHistory(did, messagesOnly); //ayu
            SQLiteCursor cursor = null;
            ...
            try {
                ...
                while (cursor.next()) {
                    ...
                    if (data != null) {
                        ...
                        message.readAttachPath(data, getUserConfig().clientUserId);
                        data.reuse();
                        //ayu: keep a permanent copy of the self-destructing media before the file is wiped
                        if (org.telegram.messenger.ayu.AyuConfig.saveSelfDestructingMedia && message.media != null) {
                            message.id = cursor.intValue(1);
                            message.dialog_id = cursor.longValue(3);
                            org.telegram.messenger.ayu.AyuMessagesController.getInstance().onSelfDestructMediaOpened(currentAccount, message, true);
                        }
...
//                        cursor = null;
    ...
    //ayu: read the messages that are about to be deleted and hand them to the AyuGram history storage
    private void ayuSaveDeletedMessages(long dialogId, ArrayList<Integer> messages, int mode) {
        if (!org.telegram.messenger.ayu.AyuConfig.saveDeletedMessages) {
            return;
        }
        if (mode != ChatActivity.MODE_DEFAULT || messages == null || messages.isEmpty()) {
            return;
        }
        if (DialogObject.isEncryptedDialog(dialogId)) {
            return;
        ...
        if (dialogId != 0) {
            ...
            ayuArchiveQuery(String.format(Locale.US, "SELECT data, mid, uid FROM messages_v2 WHERE mid IN(%s) AND is_channel = 0", ayuIds));
        }
    }

    //ayu: same as above, for the "delete everything up to mid" channel variant
    private void ayuSaveDeletedChannelMessages(long channelId, int maxMid) {
        if (!org.telegram.messenger.ayu.AyuConfig.saveDeletedMessages) {
            return;
        }
    ...
    //ayu: shared reader - deserializes "SELECT data, mid, uid ..." rows and hands them to the AyuGram history storage
    private void ayuArchiveQuery(String query) {
        SQLiteCursor ayuCursor = null;
        try {
            ArrayList<TLRPC.Message> oldMessages = new ArrayList<>();
            ayuCursor = database.queryFinalized(query);
            while (ayuCursor.next()) {
                NativeByteBuffer data = ayuCursor.byteBufferValue(0);
                if (data == null) {
                    continue;
                }
                try {
                    ...
                    message.readAttachPath(data, getUserConfig().clientUserId);
                    message.id = ayuCursor.intValue(1);
                    message.dialog_id = ayuCursor.longValue(2);
            ...
            if (!oldMessages.isEmpty()) {
                org.telegram.messenger.ayu.AyuMessagesController.getInstance().onMessagesDeleted(currentAccount, oldMessages);
            }
        } catch (Exception e) {
            FileLog.e(e);
        } finally {
    ...
    private ArrayList<Long> markMessagesAsDeletedInternal(long dialogId, ArrayList<Integer> messages, boolean deleteFiles, int mode, int threadMessageId) {
        SQLiteCursor cursor = null;
        SQLitePreparedStatement state = null;
        ayuSaveDeletedMessages(dialogId, messages, mode); //ayu
        try {
...
//                }
    ...
    private ArrayList<Long> markMessagesAsDeletedInternal(long channelId, int mid, boolean deleteFiles) {
        SQLiteCursor cursor = null;
        SQLitePreparedStatement state = null;
        ayuSaveDeletedChannelMessages(channelId, mid); //ayu
        try {
            ArrayList<Long> dialogsIds = new ArrayList<>();
            LongSparseArray<Integer[]> dialogsToUpdate = new LongSparseArray<>();
